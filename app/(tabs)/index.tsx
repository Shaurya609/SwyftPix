...
export default function HomeScreen() {
    ...
    async function loadMore() {
      ...
      try {
        ...
        setItems(prev => {
          ...
          return [...prev, ...result.items.filter(item => matchesCategory(item, selectedCategory) && !existingIds.has(item.id) && !reviewedIds.has(item.id))];
        });
        setEndCursor(result.endCursor);
        setHasNextPage(result.hasNextPage);
      } catch (err) {
  ...
  const categorySelection = (
    <View style={styles.categoryContainer}>
      <ThemedText style={styles.categoryHeading}>What do you want to clean?</ThemedText>
      <ThemedText style={styles.categoryDescription} lightColor="#687076" darkColor="#9BA1A6">Choose a media type to enter swipe mode.</ThemedText>
      <View style={styles.categoryGrid}>
        {CATEGORIES.map(category => (
          <TouchableOpacity key={category.id} style={[styles.categoryCard, isDark ? styles.categoryCardDark : styles.categoryCardLight]} onPress={() => handleSelectCategory(category.id)} activeOpacity={0.82}>
            <View style={[styles.categoryIcon, isDark ? styles.categoryIconDark : styles.categoryIconLight]}><MaterialIcons name={category.icon as any} size={30} color="#0a7ea4" /></View>
      ...
      <ThemedText style={styles.categoryNote} lightColor="#8E8E93" darkColor="#8E8E93">More file categories (documents, APKs and archives) will appear as those providers are added.</ThemedText>
    </View>
  );
    ...
    <GestureHandlerRootView style={styles.container}>
      <ThemedView style={[styles.screen, { paddingTop: insets.top, paddingBottom: insets.bottom || 16 }]}> 
        ...
        {hasPermission && hasMediaManagementAccess === false ? (
          <View style={styles.cardContainer}><View style={[styles.emptyCard, isDark ? styles.emptyCardDark : styles.emptyCardLight]}>
          ...
          <View style={styles.categoryArea}>{categorySelection}</View>
        ) : (
          <>
            <View style={styles.modeHeader}>
              <TouchableOpacity onPress={handleChangeCategory} style={styles.changeCategoryButton} activeOpacity={0.75}><MaterialIcons name="arrow-back" size={20} color="#0a7ea4" /><ThemedText style={styles.changeCategoryText}>Categories</ThemedText></TouchableOpacity>
              <View style={styles.modeTitleContainer}><ThemedText style={styles.modeTitle}>{CATEGORIES.find(c => c.id === selectedCategory)?.label}</ThemedText><ThemedText style={styles.modeSubtitle} lightColor="#687076" darkColor="#9BA1A6">Swipe to decide what stays</ThemedText></View>
            </View>
            <View style={styles.cardContainer}>
              {isLoadingDeviceMedia && items.length === 0 ? <ActivityIndicator size="large" color="#0a7ea4" /> : items.length > 0 ? items.slice(0, 2).reverse().map((item, index) => {
                ...
                <View style={styles.emptyContainer}><View style={[styles.emptyCard, isDark ? styles.emptyCardDark : styles.emptyCardLight]}>
                  <View style={styles.emptyIconContainer}><MaterialIcons name="celebration" size={44} color="#34C759" /></View>
                  <ThemedText style={styles.emptyTitle}>All Caught Up!</ThemedText>
                  <ThemedText style={styles.emptyDescription} lightColor="#687076" darkColor="#9BA1A6">You have finished reviewing everything in {CATEGORIES.find(c => c.id === selectedCategory)?.label.toLowerCase()}.</ThemedText>
                  <View style={styles.statsContainer}>
                    <View style={styles.statRow}><ThemedText style={styles.statLabel} lightColor="#687076" darkColor="#9BA1A6">Space Cleaned</ThemedText><ThemedText style={styles.statValue} lightColor="#34C759" darkColor="#30D158">{formatFileSize(spaceSaved)}</ThemedText></View>
                    <View style={styles.statRow}><ThemedText style={styles.statLabel} lightColor="#687076" darkColor="#9BA1A6">Files Deleted</ThemedText><ThemedText style={styles.statValue}>{deletedItems.length}</ThemedText></View>
                    <View style={styles.statRow}><ThemedText style={styles.statLabel} lightColor="#687076" darkColor="#9BA1A6">Files Kept</ThemedText><ThemedText style={styles.statValue}>{keptItems.length}</ThemedText></View>
                  </View>
                  <TouchableOpacity style={styles.resetButton} onPress={handleReset} activeOpacity={0.8}><MaterialIcons name="replay" size={20} color="#FFF" /><Text style={styles.resetButtonText}>Reset and Restart</Text></TouchableOpacity>
                </View></View>
              )}
            </View>
            {items.length > 0 && <View style={styles.buttonsContainer}>
              <TouchableOpacity onPress={handleUndo} disabled={!history.length} style={[styles.roundButton, isDark && styles.roundButtonDark, styles.undoButton, !history.length && styles.disabledButton]} activeOpacity={0.7}><MaterialIcons name="undo" size={22} color={!history.length ? (isDark ? '#48484A' : '#AEAEB2') : '#FF9500'} /></TouchableOpacity>
              <TouchableOpacity onPress={() => cardRef.current?.swipeLeft()} style={[styles.roundButton, isDark && styles.roundButtonDark, styles.deleteButton]} activeOpacity={0.7}><MaterialIcons name="close" size={32} color="#FF3B30" /></TouchableOpacity>
              <TouchableOpacity onPress={() => cardRef.current?.swipeRight()} style={[styles.roundButton, isDark && styles.roundButtonDark, styles.keepButton]} activeOpacity={0.7}><MaterialIcons name="check" size={32} color="#34C759" /></TouchableOpacity>
            </View>}
          </>
        )}
      </ThemedView>
...
const styles = StyleSheet.create({
  ...
  categoryArea: { flex: 1, paddingHorizontal: 16, paddingTop: 18 },
  categoryContainer: { flex: 1, justifyContent: 'center' }, categoryHeading: { fontSize: 25, fontWeight: '800', textAlign: 'center', marginBottom: 6 }, categoryDescription: { fontSize: 14, textAlign: 'center', marginBottom: 22 },
  categoryGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 12, justifyContent: 'center' },
  categoryCard: { width: '47%', minHeight: 145, borderRadius: 22, padding: 18, borderWidth: 1, justifyContent: 'center', alignItems: 'center', shadowOffset: { width: 0, height: 5 }, shadowOpacity: 0.08, shadowRadius: 9, elevation: 3 },
  categoryCardLight: { backgroundColor: '#FFFFFF', borderColor: '#E5E5EA', shadowColor: '#000000' }, categoryCardDark: { backgroundColor: '#1C1C1E', borderColor: '#2C2C2E', shadowColor: '#000000' },
  categoryIcon: { width: 58, height: 58, borderRadius: 29, justifyContent: 'center', alignItems: 'center', marginBottom: 10 }, categoryIconLight: { backgroundColor: '#EAF7FB' }, categoryIconDark: { backgroundColor: '#16313A' },
  categoryLabel: { fontSize: 17, fontWeight: '700', marginBottom: 3 }, categorySubtitle: { fontSize: 11, textAlign: 'center' }, categoryNote: { fontSize: 11, textAlign: 'center', lineHeight: 16, marginTop: 20, paddingHorizontal: 20 },
  modeHeader: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 18, paddingTop: 12, paddingBottom: 2 }, changeCategoryButton: { flexDirection: 'row', alignItems: 'center', gap: 5, paddingVertical: 8, paddingRight: 8 }, changeCategoryText: { color: '#0a7ea4', fontSize: 13, fontWeight: '700' }, modeTitleContainer: { flex: 1, alignItems: 'center', marginRight: 75 }, modeTitle: { fontSize: 18, fontWeight: '800' }, modeSubtitle: { fontSize: 11, marginTop: 2 },
  cardContainer: { flex: 1, marginHorizontal: 16, marginVertical: 8, position: 'relative', justifyContent: 'center', alignItems: 'center' },
  buttonsContainer: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 20, paddingBottom: 12, paddingHorizontal: 20 },
  ...
  roundButtonDark: { backgroundColor: '#1C1C1E', borderColor: '#2C2C2E' }, undoButton: { width: 48, height: 48, borderRadius: 24 }, deleteButton: { width: 64, height: 64, borderRadius: 32, borderColor: 'rgba(255, 59, 48, 0.2)' }, keepButton: { width: 64, height: 64, borderRadius: 32, borderColor: 'rgba(52, 199, 89, 0.2)' }, disabledButton: { opacity: 0.4 },
  ...
  emptyIconContainer: { width: 72, height: 72, borderRadius: 36, backgroundColor: 'rgba(52, 199, 89, 0.15)', justifyContent: 'center', alignItems: 'center', marginBottom: 16 }, emptyTitle: { fontSize: 20, fontWeight: '700', textAlign: 'center', marginBottom: 8 }, emptyDescription: { fontSize: 13, textAlign: 'center', lineHeight: 18, marginBottom: 20 }, statsContainer: { width: '100%', gap: 12, marginBottom: 20 }, statRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: 'rgba(128, 128, 128, 0.1)' }, statLabel: { fontSize: 13, fontWeight: '500' }, statValue: { fontSize: 14, fontWeight: '600' },
