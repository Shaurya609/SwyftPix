...
function AudioPreview({ uri }: { uri: string }) {
  ...
  return (
    <View style={styles.audioPreview}>
      ...
      <Text style={styles.typeLabel}>AUDIO FILE</Text>
      ...
      <View style={styles.container}>
        <View style={styles.header}>
          <View style={styles.meta}>
            ...
            <Text style={styles.subtitle}>{formatFileSize(item.fileSize)} • {formatDate(item.dateCreated)}</Text>
          </View>
        ...
        <View style={styles.actionBar}>
          <TouchableOpacity style={[styles.action, styles.restoreAction]} onPress={() => onRestore(item)} disabled={isProcessing}>
            <MaterialIcons name="restore" size={21} color="#FFFFFF" />
            <Text style={styles.actionText}>Restore</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.action, styles.deleteAction]} onPress={handleDelete} disabled={isProcessing}>
            <MaterialIcons name="delete-forever" size={21} color="#FFFFFF" />
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 18, paddingTop: 50, paddingBottom: 14, backgroundColor: '#080808', zIndex: 10 },
  meta: { flex: 1, marginRight: 14 },
  fileName: { color: '#FFFFFF', fontSize: 16, fontWeight: '700' },
  subtitle: { color: '#A0A0A0', fontSize: 12, marginTop: 4 },
  closeButton: { width: 44, height: 44, borderRadius: 22, backgroundColor: '#292929', justifyContent: 'center', alignItems: 'center' },
  content: { flex: 1, justifyContent: 'center', alignItems: 'center' },
  fullMedia: { width: '100%', height: '100%' },
  actionBar: { flexDirection: 'row', gap: 12, paddingHorizontal: 18, paddingTop: 12, paddingBottom: 28, backgroundColor: '#080808' },
  action: { flex: 1, minHeight: 52, borderRadius: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 },
  restoreAction: { backgroundColor: '#0A7EA4' },
  deleteAction: { backgroundColor: '#FF3B30' },
  actionText: { color: '#FFFFFF', fontSize: 14, fontWeight: '800' },
  processing: { ...StyleSheet.absoluteFillObject, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.35)' },
  ...
  typeLabel: { color: '#FFFFFF', fontSize: 14, fontWeight: '800', letterSpacing: 2, marginBottom: 28 },
  ...
  audioPlayButton: { width: 70, height: 70, borderRadius: 35, backgroundColor: '#6C5CE7', justifyContent: 'center', alignItems: 'center' },
  audioTimeline: { width: '100%', paddingVertical: 8 },
  audioTrack: { height: 6, borderRadius: 3, backgroundColor: '#444444', position: 'relative' },
  audioProgress: { height: 6, borderRadius: 3, backgroundColor: '#FFFFFF' },
  audioThumb: { position: 'absolute', top: -5, marginLeft: -8, width: 16, height: 16, borderRadius: 8, backgroundColor: '#FFFFFF' },
  audioTimeRow: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 8 },
  audioTime: { color: '#B0B0B0', fontSize: 12 },
});