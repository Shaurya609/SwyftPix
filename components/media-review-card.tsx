...
const FullscreenAudioPreview = ({ uri }: { uri: string }) => {
  ...
  return <View style={styles.audioPreview}>
    ...
    <TouchableOpacity style={styles.audioTimeline} onLayout={e => setTimelineWidth(e.nativeEvent.layout.width)} onPress={e => seekToPosition(e.nativeEvent.locationX)}>
      <View style={styles.audioTrack}><View style={[styles.audioProgress, { width: `${progress * 100}%` }]} /><View style={[styles.audioThumb, { left: `${progress * 100}%` }]} /></View>
...
const styles = StyleSheet.create({
  ...
  audioTimeline: { width: '100%', paddingVertical: 8 }, audioTrack: { height: 6, borderRadius: 3, backgroundColor: '#444444', position: 'relative' }, audioProgress: { height: 6, borderRadius: 3, backgroundColor: '#FFFFFF' }, audioThumb: { position: 'absolute', top: -5, marginLeft: -8, width: 16, height: 16, borderRadius: 8, backgroundColor: '#FFFFFF' }, audioTimeRow: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 8 }, audioTime: { color: '#B0B0B0', fontSize: 12 },
});