  ...
  return (
    <View style={styles.container}>
      <View style={styles.controlsRow}>
        <TouchableOpacity style={styles.skipButton} onPress={() => seekBy(-10)} activeOpacity={0.8} accessibilityRole="button" accessibilityLabel="Back 10 seconds">
          <MaterialIcons name="replay-10" size={28} color="#6C5CE7" />
        </TouchableOpacity>
        <TouchableOpacity style={styles.playButton} onPress={togglePlayback} activeOpacity={0.8} accessibilityRole="button" accessibilityLabel={status.playing ? 'Pause audio' : 'Play audio'}>
          <MaterialIcons name={status.playing ? 'pause' : 'play-arrow'} size={42} color="#FFFFFF" style={status.playing ? undefined : { marginLeft: 3 }} />
        </TouchableOpacity>
        <TouchableOpacity style={styles.skipButton} onPress={() => seekBy(10)} activeOpacity={0.8} accessibilityRole="button" accessibilityLabel="Forward 10 seconds">
...
const styles = StyleSheet.create({
  ...
  controlsRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 18, marginBottom: 18 },
  skipButton: { width: 44, height: 44, borderRadius: 22, backgroundColor: 'rgba(108, 92, 231, 0.1)', justifyContent: 'center', alignItems: 'center' },
  playButton: { width: 78, height: 78, borderRadius: 39, backgroundColor: '#6C5CE7', justifyContent: 'center', alignItems: 'center', shadowColor: '#000000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.18, shadowRadius: 8, elevation: 4 },
  timelineContainer: { width: '100%', paddingVertical: 8 },
  track: { height: 7, borderRadius: 4, overflow: 'hidden', backgroundColor: 'rgba(108, 92, 231, 0.22)' },
  progress: { height: '100%', borderRadius: 4, backgroundColor: '#6C5CE7' },
  timeRow: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 7 },
  timeText: { color: '#8E8E93', fontSize: 11, fontWeight: '600' },
});
